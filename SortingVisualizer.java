...
// yaha hum apne button create karenge
    private JPanel createControls() {
        ...
        complexityButton = new JButton("Time Complexity"); // Add button
        panel.add(startButton);
        panel.add(shuffleButton);
        panel.add(complexityButton); // Add to panel

        panel.add(new JLabel("Speed:"));
        speedSlider = new JSlider(0, 200, 40);
        ...
        // Ab hum isme action perform karenge
        ...
        // isme hum time complexity batange
        complexityButton.addActionListener(e -> {
            String algo = (String) algoBox.getSelectedItem();
            String info = getComplexityInfo(algo);
            JOptionPane.showMessageDialog(this, info, "Time Complexity", JOptionPane.INFORMATION_MESSAGE);
        });
    ...
    private void disableControlsWhileRunning(boolean disable) {
        ...
        complexityButton.setEnabled(!disable);
    }
    ...
    private int[] generateArray(int n) {
        ...
        int[] a = new int[n];
        for (int i = 0; i < n; i++)
            a[i] = 10 + r.nextInt(480);
        return a;
    }

    private int getDelayFromSlider() {
        int val = speedSlider.getValue();
        int delay = Math.max(1, 201 - val);
        return delay;
    }
    ...
    // yaha hum apne sorting Algo likhenge
    ...
    // Utility method to get complexity info
    private String getComplexityInfo(String algo) {
        switch (algo) {
            case "Bubble Sort":
                return """
                Bubble Sort:
